...
# Si usamos MySQL en nuestra máquina(localhost)
CREATE USER 'sfg_dev_user'@'localhost' IDENTIFIED BY 'guru';
CREATE USER 'sfg_prod_user'@'localhost' IDENTIFIED BY 'guru';

# Si usamos MySQL con un contenedor de Docker(Usamos el comdín '%' porque en Docker se asigna dinámicamente las IPs a los contenedores)
# '%' es un comodín que indica cualquier host
